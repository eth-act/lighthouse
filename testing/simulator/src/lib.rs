...
//!
//! The library target exposes the building blocks ([`LocalNetwork`], [`checks`] and [`retry`]) so
//! other testing crates can reuse the node orchestration instead of duplicating it. The
//! `simulator` binary in `main.rs` wires them into the `basic-sim` and `fallback-sim` subcommands.
//!
//! ## Future works
//!
