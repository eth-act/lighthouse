...
pub struct BeaconChain<T: BeaconChainTypes> {
    ...
    /// Internal event bus for test and simulation subscribers.
    ///
    /// Lazily initialised on the first call to [`Self::subscribe_internal_events`]; stays `None`
    /// in production runs where no subscriber has ever registered, making all emission sites
    /// (`if let Some(tx) = self.internal_event_tx.get()`) no-ops.
    pub internal_event_tx: OnceLock<tokio::sync::broadcast::Sender<InternalBeaconNodeEvent>>,
...
impl<T: BeaconChainTypes> BeaconChain<T> {
    ...
    ///
    /// The broadcast channel is created lazily on the first call to this method. In production
    /// runs where no subscriber registers, the channel is never created and
    /// [`Self::internal_event_sender`] returns `None`, making all emission sites no-ops.
    ...
    /// Returns the internal event sender if any subscriber has registered, otherwise `None`.
    ///
    /// Use this to gate event construction: only build the event value when a subscriber exists.
    pub fn internal_event_sender(
        &self,
    ) -> Option<&tokio::sync::broadcast::Sender<InternalBeaconNodeEvent>> {
        self.internal_event_tx.get()
    }

    /// Send an event on the internal event bus.
    ///
    /// Does nothing if no subscriber has registered. Prefer calling this after checking
    /// [`Self::internal_event_sender`]`.is_some()` so event construction is also skipped.
    pub fn emit_internal_event(&self, event: InternalBeaconNodeEvent) {
