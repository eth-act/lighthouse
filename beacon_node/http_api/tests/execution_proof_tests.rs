...
async fn submit_execution_proofs_rejections() {
    ...
    let mut forged = tester.proof(ProofType::RethZisk, &VALID_PROOF_DATA, 4);
    ...
    let valid = tester.proof(ProofType::RethZisk, &VALID_PROOF_DATA, 6);
    assert_rejected(
        client
            .post_beacon_execution_proofs(&list(vec![
                tester.proof(ProofType::RethZisk, &[0xee], 7),
                valid.clone(),
            ]))
            .await,
        &[(0, "InvalidProof")],
